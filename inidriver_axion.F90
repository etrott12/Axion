    ...
    real(dl) actual_massless,neff_i,a,a2,dorp,gr,grhoa2

!a can be defined from P%loga_table
!a2 = a**2
!dorp and grhoa2 are to be defined within the spline later
!still not sure about how gr is defined
...
!    real clock_start, clock_stop ! RH timing	
    ...
    double precision ombh2, omch2, omaxh2, mass_axion, h0, ns
...
!    print*, 'Renee reading hubble: ', P%h0
    ...
    if(P%ma .ge. 1e-15) then
       print*, 'Warning: High axion mass. Entropy production is not included, and may affect the relic abundance calculation.'
    endif
...
open(unit = 1,file = "grhoax")
!Emery -- added this section from equations_ppf.f90 to plot grhoax_table vs. a
!Spline for axion energy density of a<aosc, otherwise just use a^-3 scaling
	do i=1,ntable,1
		a = 10**P%loga_table(i)
		!a = exp(P%loga_table(i))
		a2 = a**2
		!print*,a,P%grhoax_table(i)
	        if (a .lt. P%a_osc) then
        	call spline_out(P%loga_table,P%grhoax_table,P%grhoax_table_buff,ntable,dlog10(a),gr)
        	!print*, a, 10**P%loga_table(i)
        	write(1,*) a, 10**P%grhoax_table(i)
        	...
        	if (gr .eq. gr) then
        	   ...
        	   dorp=0.0d0
        	        ...
        	        dorp=grhom*P%drefp_hsq*((P%a_osc/a)**3.0d0)
        	endif
